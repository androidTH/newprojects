...
//            }
    ...
    public void setLabels(List<String> labels, int resId) {
        setLabels(labels, new LabelTextProvider<String>() {
            @Override
            public CharSequence getLabelText(TextView label, int position, String data) {
                return data.trim();
    ...
    public <T> void setLabels(List<T> labels, LabelTextProvider<T> provider, int resId) {
        //清空原有的标签
        removeAllViews();
        mLabels.clear();
        ...
        if (labels != null) {
            ...
            int size = labels.size();
            for (int i = 0; i < size; i++) {
                addLabel(labels.get(i), i, provider, resId);
            }
    ...
    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider) {
        ...
        Drawable drawable = getResources().getDrawable(R.mipmap.comment_local_del);
        drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());// 设置边界
        label.setCompoundDrawables(null,null,drawable,null);
        label.setCompoundDrawablePadding(mDrawablePadding);
        addView(label);
        label.setText(provider.getLabelText(label, position, data));
    }

    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider, int resId) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.layout_tag, null);
        TextView label = (TextView) view.findViewById(R.id.tv_label);
        TextView IVLabelDel = (TextView) view.findViewById(R.id.iv_label_del);
        label.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSize);
        label.setTextColor(mTextColor != null ? mTextColor : ColorStateList.valueOf(0xFF000000));
        //设置给label的背景(Drawable)是一个Drawable对象的拷贝，
        // 因为如果所有的标签都共用一个Drawable对象，会引起背景错乱。
//        label.setBackgroundDrawable(mLabelBg.getConstantState().newDrawable());
    ...
    private void ensureLabelClickable(int resId) {
        int count = getChildCount();
        for (int i = 0; i < count; i++) {
            View view = getChildAt(i);
            TextView label= (TextView) view.findViewById(R.id.iv_label_del);
            label.setClickable(mLabelClickListener != null || mSelectType != SelectType.NONE);
        }
    }
    ...
    public void setOnLabelClickListener(OnLabelClickListener l,int resId) {
        mLabelClickListener = l;
        ensureLabelClickable(resId);
    }
