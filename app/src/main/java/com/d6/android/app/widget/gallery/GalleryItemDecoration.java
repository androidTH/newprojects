...
public class GalleryItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void onSetVerticalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        ...
        int itemNewHeight = parent.getHeight() - DisplayUtil.dpToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);

        mItemConsumeY = itemNewHeight + DisplayUtil.dpToPx(2 * mPageMargin);
        ...
        int topMargin = position == 0 ? DisplayUtil.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : DisplayUtil.dpToPx(mPageMargin);
        int bottomMargin = position == itemCount - 1 ? DisplayUtil.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : DisplayUtil.dpToPx(mPageMargin);
    ...
    private void onSetHorizontalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        int itemNewWidth = parent.getWidth() - DisplayUtil.dpToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);
        int itemNewHeight = parent.getHeight();

        mItemConsumeX = itemNewWidth + DisplayUtil.dpToPx(2 * mPageMargin);
        ...
        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int leftMargin = position == 0 ? DisplayUtil.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : DisplayUtil.dpToPx(mPageMargin);
        int rightMargin = position == itemCount - 1 ? DisplayUtil.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : DisplayUtil.dpToPx(mPageMargin);
