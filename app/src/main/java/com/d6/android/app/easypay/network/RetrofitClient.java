...
public class RetrofitClient implements NetworkClientInterf {
    ...
    public void post(PayParams payParams, final CallBack c) {
        ...
        if(payParams.getType()==1){
            call = service.postBuyFlower(payParams.getiUserid(),payParams.getiSendUserid(),Integer.valueOf(payParams.getPayWay().toString()),payParams.getGoodsPrice(),payParams.getiFlowerCount());
