...
public class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private boolean mShowLastVerticalLine;
    private int mHorizonSpan;
    private int mVerticalSpan;

    private GridItemDecoration(int horizonSpan,int verticalSpan,int color,boolean showLastLine,boolean showLastverticalline) {
        this.mHorizonSpan = horizonSpan;
        this.mShowLastLine = showLastLine;
        this.mShowLastVerticalLine = showLastverticalline;
...
//            //满足条件( 最后一行 && 不绘制 ) 将vertical多出的一部分去掉;
            if (mShowLastVerticalLine&&i==childCount-1) {
                right -= mVerticalSpan;
            }
            mDivider.setBounds(left, top, right, bottom);
            mDivider.draw(c);
    ...
    public static class Builder {
         ...
         * 是否最后一条显示分割线
         * */
        public GridItemDecoration.Builder setShowVerticalLine(boolean show){
            mShowLastVerticalLine = show;
            return this;
        }

        public GridItemDecoration build() {
            return new GridItemDecoration(mHorizonSpan, mVerticalSpan, mColor,mShowLastLine,mShowLastVerticalLine);
        }
    }
}
