...
class SwipeRefreshRecyclerLayout : SwipeRefreshLayout, SwipeRefreshLayout.OnRefreshListener {
    ...
    internal fun init() {
        ...
        loadMoreTextColor = ContextCompat.getColor(context, R.color.textColor99)
    ...
    private val onScrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrollStateChanged(recyclerView: RecyclerView?, newState: Int) {
            ...
            if (newState == RecyclerView.SCROLL_STATE_IDLE && adapter != null
                    && lastVisibleItem + 1 == adapter.itemCount && !isRefreshing && !isLoadMore && (mode==Mode.Both || mode==Mode.Bottom)) {
                if (adapter is HFRecyclerAdapter<*>) {
                    if (loadMoreView != null) {
                        loadMoreView?.text = "载入中..."
