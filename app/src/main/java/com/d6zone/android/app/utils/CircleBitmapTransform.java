...
public class CircleBitmapTransform {
    public static Bitmap transform(Bitmap toTransform) {
        ...
        Bitmap output = Bitmap.createBitmap(bitmapWidth, bitmapWidth, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);

        RectF outerRect = new RectF(0, 0, bitmapWidth, bitmapWidth);
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        // paint.setColor()的参数，除不能为Color.TRANSPARENT外，可以任意写
        paint.setColor(Color.WHITE);
        int cornerRadius = Math.min((bitmapWidth / 2), (bitmapWidth / 2));
