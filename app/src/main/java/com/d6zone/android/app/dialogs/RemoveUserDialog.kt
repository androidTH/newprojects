...
import kotlinx.android.synthetic.main.dialog_removeuser_layout.*
 ...
 * 通讯录好友
 */
class RemoveUserDialog : DialogFragment(),RequestManager {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? =
            inflater?.inflate(R.layout.dialog_removeuser_layout, container, false)

    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        tv_remove.setOnClickListener {
            dialogListener?.onClick(1,"确认移除好友")
            dismissAllowingStateLoss()
        }

        tv_cancel.setOnClickListener {
            dialogListener?.onClick(2,"取消移除好友")
            dismissAllowingStateLoss()
        }
